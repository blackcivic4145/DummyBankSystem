...
function renderMyPage() {
  const s = getState();
  // Standard card with PayPay balance if linked
  let cardHtml = `
  ...
  if (s.isPayPayLinked) {
    cardHtml += `
      <div style="margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.2);display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:12px;opacity:0.8">PayPay残高</span>
        <span style="font-size:18px;font-weight:bold">${fmt(s.paypayBalance)}</span>
      </div>`;
  }
